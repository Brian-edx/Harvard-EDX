...
function Brick:init(x, y)
    ...
    -- if locked, the key is required to destroy this block
    self.isLocked = false
    ...
    -- The following should now use ParticleSystem:setEmissionArea():
    -- self.psystem:setAreaSpread('normal', 10, 10)
    self.psystem:setEmissionArea('normal', 10, 10, 6.28, true)
end
...
function Brick:getScore()
    if self.isLocked then
        return 2000
    else
        return self.tier * 200 + self.color * 25
    end
end
